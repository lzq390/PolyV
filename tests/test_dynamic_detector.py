...
class DynamicDetectorTests(unittest.TestCase):
    def test_dynamic_rois_follow_liquid_region_without_exporting_shaft(self):
        cfg = dynamic_config()
        left_tracker = DynamicRoiTracker.calibrate([synthetic_frame(shaft_x=300) for _ in range(4)], cfg)
        right_tracker = DynamicRoiTracker.calibrate([synthetic_frame(shaft_x=340) for _ in range(4)], cfg)
        ...
        self.assertAlmostEqual(left_liquid.x0, right_liquid.x0, delta=0.01)
        self.assertAlmostEqual(left_liquid.x1, right_liquid.x1, delta=0.025)
        self.assertGreater(left_rois.scores["liquid_ellipse_score"], 0.50)
        self.assertGreaterEqual(left_liquid.x1 - left_liquid.x0, 0.30)
        self.assertLessEqual(left_liquid.x1 - left_liquid.x0, 0.45)
        self.assertGreaterEqual(left_rod.x1 - left_rod.x0, 0.12)
        self.assertLessEqual(left_rod.x1 - left_rod.x0, 0.20)
        self.assertGreaterEqual(left_rod.y0, left_liquid.y0 - 0.08)
    ...
    def test_liquid_roi_updates_after_calibration_for_reliable_ellipse(self):
        cfg = dynamic_config(
            calibration_duration_sec=2.0,
            baseline_duration_sec=2.0,
        ...
        self.assertEqual(result.evidence["rois"]["rod_roi"], calibrated_rod_roi)
        self.assertGreater(result.evidence["rois"]["liquid_roi"][1], calibrated_liquid_roi[1] + 0.07)
